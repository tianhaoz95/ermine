use crate::memory::{LayerKey, LayerWeightCache};
use crate::model::ModelConfig;
use crate::{GgmlBackend, GgmlOps, GgmlTensor};
...
impl QwenModel {
    pub fn new(
        config: ModelConfig,
        weights: HashMap<String, GgmlTensor>,
        cache: Option<Arc<LayerWeightCache>>,
    ) -> Self {
        QwenModel {
            config,
            weights,
    ...
    pub async fn forward(
        &self,
        input_ids: Tensor<GgmlBackend, 1, Int>,
        cur_pos: usize,
    ) -> Tensor<GgmlBackend, 1, Float> {
        // 1. Embedding
        ...
        for i in 0..self.config.n_layers {
            ...
            let layer_tensors = if let Some(cache) = &self.cache {
                ...
                slot.tensors
                    .get()
                    .expect("Failed to load layer weights")
                    .clone()
            } else {
        ...
        let logits_tensor: Tensor<GgmlBackend, 2, Float> =
    ...
    async fn layer_forward(
        ...
        let attn_norm_weight = weights
            .get(&format!("blk.{}.attn_norm.weight", _layer_idx))
            .expect("Missing attn_norm");
        let norm_x = GgmlBackend::rms_norm(
            x.clone(),
        ...
        let w_q = weights
            .get(&format!("blk.{}.attn_q.weight", _layer_idx))
            .expect("Missing q weight");
        let q = GgmlBackend::float_matmul(norm_x, w_q.clone());

        // MLP
        let ffn_norm_weight = weights
            .get(&format!("blk.{}.ffn_norm.weight", _layer_idx))
            .expect("Missing ffn_norm");
        let norm_x_ffn =
            GgmlBackend::rms_norm(q, ffn_norm_weight.clone(), self.config.rms_norm_eps);

        let w_gate = weights
            .get(&format!("blk.{}.ffn_gate.weight", _layer_idx))
            .expect("Missing gate weight");
