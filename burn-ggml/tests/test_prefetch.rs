...
use burn_ggml::{get_prefetch_count, GgmlBackend, GgmlDevice, TensorPrefetch};
...
fn test_tensor_prefetch_updates_count() {
    ...
    assert_eq!(
        final_count,
        initial_count + 1,
        "Prefetch count should have increased by 1"
    );
    ...
    assert_eq!(
        get_prefetch_count(),
        initial_count + 3,
        "Prefetch count should have increased by 3 total"
    );
}
