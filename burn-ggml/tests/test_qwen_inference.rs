...
use burn::tensor::{Float, Int, Tensor, TensorPrimitive};
use burn_ggml::memory::LayerKey;
use burn_ggml::{gguf::GgufIndex, GgmlBackend, GgmlContext, GgmlDevice, GgmlOps};
use std::path::Path;
...
async fn test_qwen_inference_burn_ggml() {
    ...
    let embd_primitive = unsafe {
        index
            .load_tensor("token_embd.weight", &ctx)
            .expect("Failed to load token_embd.weight")
    };
    let embd_weight: Tensor<GgmlBackend, 2, Float> =
        Tensor::from_primitive(TensorPrimitive::Float(embd_primitive));
    ...
    let norm_weight_primitive = tensors0
        .get("blk.0.attn_norm.weight")
        .expect("Failed to find blk.0.attn_norm.weight in cache");
    let norm_weight: Tensor<GgmlBackend, 1, Float> =
        Tensor::from_primitive(TensorPrimitive::Float(norm_weight_primitive.clone()));
    ...
    let x_primitive = GgmlBackend::embedding(
        embd_weight.into_primitive().tensor(),
        input_ids.into_primitive(),
    );
    let x: Tensor<GgmlBackend, 2, Float> =
        Tensor::from_primitive(TensorPrimitive::Float(x_primitive));
    ...
    let out: Tensor<GgmlBackend, 2, Float> =
        Tensor::from_primitive(TensorPrimitive::Float(out_primitive));

    // 5. Verify Output
    let data = out.into_data();
    let values = data.as_slice::<f32>().unwrap();
    println!(
        "Inference output (first 10 values): {:?}",
        &values[..std::cmp::min(values.len(), 10)]
    );
