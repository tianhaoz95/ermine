...
use burn_ggml::{OffloadBackend, TensorPrefetch, OFFLOAD_PREFETCH_COUNT};
...
fn test_wgpu_prefetch_simulated_improvement() {
    ...
    assert_eq!(
        OFFLOAD_PREFETCH_COUNT.load(Ordering::SeqCst),
        0,
        "Initially 0 prefetch calls"
    );
    ...
    assert_eq!(
        OFFLOAD_PREFETCH_COUNT.load(Ordering::SeqCst),
        1,
        "Prefetch should increment count"
    );
    ...
    assert_eq!(
        OFFLOAD_PREFETCH_COUNT.load(Ordering::SeqCst),
        2,
        "Second prefetch should increment count again"
    );
}
