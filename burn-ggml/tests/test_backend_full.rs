use burn::tensor::{Float, Tensor, TensorData, TensorPrimitive, Tolerance};
use burn_ggml::{gguf::GgufIndex, GgmlBackend, GgmlDevice};
use std::io::Write;
...
"#;
    ...
    let mut script_file =
        std::fs::File::create("create_test_model_tmp.py").expect("Failed to create script file");
    script_file
    ...
    let weight_primitive = unsafe {
        index
            .load_tensor("test.weight", &ctx)
            .expect("Failed to load tensor")
    };
    let weight: Tensor<GgmlBackend, 2, Float> =
        Tensor::from_primitive(TensorPrimitive::Float(weight_primitive));
    ...
    sum_data.assert_approx_eq(
        &TensorData::from([[11.0, 22.0], [33.0, 44.0]]),
