...
use burn_ggml::memory::{WeightCache, LayerKey};
use burn::tensor::{Tensor, Float, TensorPrimitive, TensorData};
use std::sync::Arc;
...
async fn test_weight_cache_loading() {
    ...
    let tensor0: Tensor<GgmlBackend, 1, Float> = Tensor::from_primitive(TensorPrimitive::Float(t0.clone()));
    ...
    let tensor1: Tensor<GgmlBackend, 1, Float> = Tensor::from_primitive(TensorPrimitive::Float(t1.clone()));
