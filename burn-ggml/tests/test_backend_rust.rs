use burn::tensor::{Float, Tensor, TensorMetadata, TensorPrimitive};
...
fn test_rust_backend_with_qwen_weights() {
    ...
    let weight_primitive = unsafe {
        index
            .load_tensor(weight_name, &ctx)
            .expect("Failed to load tensor")
    };

    println!(
        "Weight loaded. Shape: {:?}, Type: {:?}",
        weight_primitive.shape(),
        weight_primitive.dtype()
    );
    ...
    let weight: Tensor<GgmlBackend, 1, Float> =
    ...
    assert!(
        sum != 0.0,
        "Backend returned all zeros! Weights likely failed to sync to backend memory."
    );
